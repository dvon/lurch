...
int simplify_gtrans() /* simplifies the transitions */
{
  ...
  if(tl_stats) {
    times(&t_fin);
    fprintf(tl_out, "Simplification of the generalized Buchi automaton - transitions: %.2fs %i transitions removed\n", 
...
int simplify_gstates() /* eliminates redundant states */
{
  ...
  if(tl_stats) {
    times(&t_fin);
    fprintf(tl_out, "\nSimplification of the generalized Buchi automaton - states: %.2fs %i states removed\n", 
	   ((float)(t_fin.tms_utime - t_debut.tms_utime))/100, changed);
  }
...
{ /* generates a generalized Buchi automaton from the alternating automaton */
  ...
  if(tl_stats) {
    times(&t_fin);
    fprintf(tl_out, "\nBuilding of the generalized Buchi automaton : %.2fs %i states, %i transitions\n", 
